...
FilterMenu::FilterMenu(const QStringList &filters, QWidget *parent, QToolButton *tool_bt) : QToolButton(parent)
{
    m_tool_bt = tool_bt;
    m_tool_bt->setPopupMode(QToolButton::InstantPopup);
    m_tool_bt->setArrowType(Qt::DownArrow);

    if(filters.size() != 0) {
        menu = new QMenu(this);
        m_tool_bt->setMenu(menu);
